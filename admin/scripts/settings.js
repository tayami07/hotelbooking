...
//function to keep values same when cancelled
...
function upd_contacts() {
    ...
    for (i = 0; i < index.length; i++) {
        data_str += index[i] + "=" + document.getElementById(contacts_inp_id[i]).value + '&';    //fetching id then accessing value
    }
    ...
    xhr.onload = function () {
        ...
        var myModal = document.getElementById('contacts-s') //fetching modal id
        var modal = bootstrap.Modal.getInstance(myModal); //calling getInstance function and passing id to var
        modal.hide();

        if (this.responseText == 1) {
            alert('success', 'Changes have been shutdown');
            get_contacts();
        }
        else {
...
function add_member()
{
    ...
    xhr.onload = function () {
        var myModal = document.getElementById('team-s') //fetching modal id
        var modal = bootstrap.Modal.getInstance(myModal); //calling getInstance function and passing id to var
        modal.hide();
